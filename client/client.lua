...
-- billing menu
RegisterNetEvent('qc-billing:client:billingMenu', function()
    ...
    local BillsOptions = {
        ...
        },
        {
            ...
            txt    = '',
            params = { event = 'qc-billing:client:billplayer' }
        },
        {
            header = 'View Your Bills',
            icon = 'fas fa-dollar-sign',
            txt    = '',
            params = { event = 'qc-billing:client:checkbills' }
        },
        {
            header = 'Cancel Sent Bill',
            icon = 'fas fa-dollar-sign',
            txt    = '',
            params = { event = 'qc-billing:client:deletebills' }
        },
        {
            header = 'close',
...
RegisterNetEvent('qc-billing:client:billplayer', function()
...
RegisterNetEvent('qc-billing:client:checkbills', function()
    local PlayerId = GetPlayerServerId(PlayerId())
    RSGCore.Functions.TriggerCallback('qc-billing:server:checkbills', function(bills, cid)
        local BillsShow = {
            {
        ...
        for _, v in ipairs(bills) do
            BillsShow[#BillsShow + 1] = {
                ...
                params = { event = 'qc-billing:server:paybills', 
                    isServer = true,
                    args = {
                        sender = v.sender, 
                        amount = v.amount, 
...
RegisterNetEvent('qc-billing:client:deletebills', function()

    RSGCore.Functions.TriggerCallback('qc-billing:server:checkSentBills', function(sentbills, citizenid)
        ...
        for _, v in ipairs(sentbills) do
            SentBillsShow[#SentBillsShow + 1] = {
                ...
                params = { event = 'qc-billing:client:cancelbill', 
                    isServer = false,
                    args = {
                        billid = v.id,
                    } 
...
-- cancel bill confirm
RegisterNetEvent('qc-billing:client:cancelbill', function(data)
    local dialog = exports['rsg-input']:ShowInput({
        header = "Cancel Bill",
        submitText = "Submit",
        inputs = {
    ...
    if dialog ~= nil then
        ...
        if dialog.cancelbill == 'yes' then
            TriggerServerEvent('qc-billing:server:cancelbill', tonumber(data.billid))
